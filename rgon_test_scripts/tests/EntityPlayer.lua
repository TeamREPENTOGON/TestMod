...
function EntityPlayerTest:TestAddCollectible(player)
	test:AddOneTimeCallback(ModCallbacks.MC_PRE_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 1)
		test.AssertEquals(charge, 0)
		test.AssertEquals(firsttime, true)
	...
	test:AddOneTimeCallback(ModCallbacks.MC_PRE_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 2)
		test.AssertEquals(charge, 0)
		test.AssertEquals(firsttime, true)
		test.AssertEquals(slot, 0)
		test.AssertEquals(vardata, 0)
	end)
	test:AddOneTimeCallback(ModCallbacks.MC_POST_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 2)
		test.AssertEquals(charge, 0)
		test.AssertEquals(firsttime, true)
	...
	test:AddOneTimeCallback(ModCallbacks.MC_PRE_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 1)
		test.AssertEquals(charge, 2)
		test.AssertEquals(firsttime, false)
		test.AssertEquals(slot, 3)
		test.AssertEquals(vardata, 4)
		return {5, 6, true, 7, 8}
	end)
	test:AddOneTimeCallback(ModCallbacks.MC_PRE_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 5)
		test.AssertEquals(charge, 6)
		test.AssertEquals(firsttime, true)
		test.AssertEquals(slot, 7)
		test.AssertEquals(vardata, 8)
	end)
	test:AddOneTimeCallback(ModCallbacks.MC_POST_ADD_COLLECTIBLE, function(_, id, charge, firsttime, slot, vardata, p)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 5)
	...
	player:AddCollectible(1, 2, false, 3, 4)
end
...
function EntityPlayerTest:TestAddTrinket(player)
	test:AddOneTimeCallback(ModCallbacks.MC_PRE_ADD_TRINKET, function(_, p, id, firsttime)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 1)
		test.AssertEquals(firsttime, true)
		return 2
	...
	test:AddOneTimeCallback(ModCallbacks.MC_POST_TRIGGER_TRINKET_ADDED, function(_, p, id, firsttime)
		test.AssertEquals(GetPtrHash(p), GetPtrHash(player))
		test.AssertEquals(id, 2)
		test.AssertEquals(firsttime, true)
	end)
...
function EntityPlayerTest:TestAddSmeltedTrinket(player)
	player:AddSmeltedTrinket(TrinketType.TRINKET_WOODEN_CROSS, true)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY | TrinketType.TRINKET_GOLDEN_FLAG, false)

	local smelted = player:GetSmeltedTrinkets()
	test.AssertEquals(smelted[TrinketType.TRINKET_WOODEN_CROSS].trinketAmount, 1)
	test.AssertEquals(smelted[TrinketType.TRINKET_WOODEN_CROSS].goldenTrinketAmount, 0)
	test.AssertEquals(smelted[TrinketType.TRINKET_SWALLOWED_PENNY].trinketAmount, 0)
	test.AssertEquals(smelted[TrinketType.TRINKET_SWALLOWED_PENNY].goldenTrinketAmount, 1)

	test.AssertTrue(player:GetEffects():HasCollectibleEffect(CollectibleType.COLLECTIBLE_HOLY_MANTLE))

	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)

	smelted = player:GetSmeltedTrinkets()
	test.AssertEquals(smelted[TrinketType.TRINKET_WOODEN_CROSS].trinketAmount, 1)
	test.AssertEquals(smelted[TrinketType.TRINKET_WOODEN_CROSS].goldenTrinketAmount, 0)
	test.AssertEquals(smelted[TrinketType.TRINKET_SWALLOWED_PENNY].trinketAmount, 0)
	test.AssertEquals(smelted[TrinketType.TRINKET_SWALLOWED_PENNY].goldenTrinketAmount, 0)

	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_WOODEN_CROSS)
	test.AssertEquals(player:GetSmeltedTrinkets()[TrinketType.TRINKET_WOODEN_CROSS].trinketAmount, 0)
end

function EntityPlayerTest:TestGetSmeltedTrinkets(player)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY | TrinketType.TRINKET_GOLDEN_FLAG)
	player:AddSmeltedTrinket(TrinketType.TRINKET_PETRIFIED_POOP)
	player:AddSmeltedTrinket(TrinketType.TRINKET_AAA_BATTERY | TrinketType.TRINKET_GOLDEN_FLAG)
	
	local all = player:GetSmeltedTrinkets()
	test.AssertEquals(all[TrinketType.TRINKET_SWALLOWED_PENNY].trinketAmount, 2)
	test.AssertEquals(all[TrinketType.TRINKET_SWALLOWED_PENNY].goldenTrinketAmount, 1)
	test.AssertEquals(all[TrinketType.TRINKET_PETRIFIED_POOP].trinketAmount, 1)
	test.AssertEquals(all[TrinketType.TRINKET_PETRIFIED_POOP].goldenTrinketAmount, 0)
	test.AssertEquals(all[TrinketType.TRINKET_AAA_BATTERY].trinketAmount, 0)
	test.AssertEquals(all[TrinketType.TRINKET_AAA_BATTERY].goldenTrinketAmount, 1)
	test.AssertEquals(all[TrinketType.TRINKET_BROKEN_MAGNET].trinketAmount, 0)
	test.AssertEquals(all[TrinketType.TRINKET_BROKEN_MAGNET].goldenTrinketAmount, 0)
	
	local filtered = player:GetSmeltedTrinkets({
		TrinketType.TRINKET_SWALLOWED_PENNY,
		TrinketType.TRINKET_PETRIFIED_POOP | TrinketType.TRINKET_GOLDEN_FLAG,
		TrinketType.TRINKET_AAA_BATTERY,
	})
	test.AssertEquals(filtered[TrinketType.TRINKET_SWALLOWED_PENNY].trinketAmount, 2)
	test.AssertEquals(filtered[TrinketType.TRINKET_SWALLOWED_PENNY].goldenTrinketAmount, 1)
	test.AssertEquals(filtered[TrinketType.TRINKET_PETRIFIED_POOP].trinketAmount, 1)
	test.AssertEquals(filtered[TrinketType.TRINKET_PETRIFIED_POOP].goldenTrinketAmount, 0)
	test.AssertEquals(filtered[TrinketType.TRINKET_AAA_BATTERY].trinketAmount, 0)
	test.AssertEquals(filtered[TrinketType.TRINKET_AAA_BATTERY].goldenTrinketAmount, 1)
	test.AssertNil(filtered[TrinketType.TRINKET_BROKEN_MAGNET])
	test.AssertNil(filtered[TrinketType.TRINKET_NULL])
	
	local subfiltered = player:GetSmeltedTrinkets({TrinketType.TRINKET_SWALLOWED_PENNY})
	test.AssertEquals(subfiltered[TrinketType.TRINKET_SWALLOWED_PENNY].trinketAmount, 2)
	test.AssertEquals(subfiltered[TrinketType.TRINKET_SWALLOWED_PENNY].goldenTrinketAmount, 1)
	test.AssertNil(subfiltered[TrinketType.TRINKET_PETRIFIED_POOP])
	test.AssertNil(subfiltered[TrinketType.TRINKET_AAA_BATTERY])
	
	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_PETRIFIED_POOP)
	player:TryRemoveSmeltedTrinket(TrinketType.TRINKET_AAA_BATTERY)
end

function EntityPlayerTest:TestGetSmeltedTrinketDesc(player)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY)
	player:AddSmeltedTrinket(TrinketType.TRINKET_SWALLOWED_PENNY | TrinketType.TRINKET_GOLDEN_FLAG)
	player:AddSmeltedTrinket(TrinketType.TRINKET_PETRIFIED_POOP)
	...
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_SWALLOWED_PENNY).trinketAmount, 2)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_SWALLOWED_PENNY).goldenTrinketAmount, 1)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_PETRIFIED_POOP).trinketAmount, 1)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_PETRIFIED_POOP).goldenTrinketAmount, 0)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_AAA_BATTERY).trinketAmount, 0)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_AAA_BATTERY).goldenTrinketAmount, 1)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_BROKEN_MAGNET).trinketAmount, 0)
	test.AssertEquals(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_BROKEN_MAGNET).goldenTrinketAmount, 0)
	test.AssertNil(player:GetSmeltedTrinketDesc(TrinketType.TRINKET_NULL))
